...
const checkDocblocks = (filePath: string) => {
    ...
    const missingDocblocks: { name: string; kind: string; file: string; line: number; reason: string }[] = [];

    const checkNode = (node: ts.Node) => {
        // Declarations can live in any of the re-exported files, so resolve lines against the
        // node's own source file rather than the entry file.
        ...
        if (
                    ...
                    node.declarationList.declarations.forEach((declaration) => {
                        ...
                        if (declSymbol) {
                            ...
                            if (docStatus.hasProblem) {
                                const name = declaration.name.getText(nodeSourceFile);
                                const line = nodeSourceFile.getLineAndCharacterOfPosition(
            ...
            const line = nodeSourceFile.getLineAndCharacterOfPosition(node.getStart()).line + 1;
    ...
    // The entry file is just a barrel of re-exports, so walk the actual exported symbols and follow
    // their aliases to wherever they're really declared.
    const moduleSymbol = checker.getSymbolAtLocation(sourceFile);
    if (!moduleSymbol) {
        throw new Error(`Could not resolve the module symbol for: ${filePath}`);
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            result.missingDocblocks.forEach((item) => {
                console.log(`  - ${item.kind} '${item.name}' at ${item.file}:${item.line}: ${item.reason}`);
            });
